import string, random, requests, time
...
for i in range(10000):
    ...
    data = f"birth_day=1&birth_month=01&birth_year=1970&collect_personal_info=undefined&creation_flow=&creation_point=https://www.spotify.com/uk/&displayname=unknown&email={email}&gender=neutral&iagree=1&key=a1e486e2729f46d6bb368d6b2bcda326&password={password}&password_repeat={password}&platform=www&referrer=&send-email=1&thirdpartyemail=0&fb=0"
    ...
    create = session.post("https://spclient.wg.spotify.com/signup/public/v1/account/", data=data, headers=headers)
    ...
    followuser = session.put("https://api.spotify.com/v1/me/following?type=user&ids="+ idd, headers=headers)
    accountstats = requests.get(f"https://spclient.wg.spotify.com/user-profile-view/v3/profile/{idd}?playlist_limit=0&artist_limit=0&episode_limit=0&market=from_token", headers=headers).json()
